...
class UserDetailDongTaiCell: UITableViewCell, RegisterCellFromNib {
    ...
    var dongtai = UserDetailDongtai() {
        didSet {
            ...
            case .postVideoOrArticle, .postVideo, .proposeQuestion, .answerQuestion, .forwardArticle, .postContentAndVideo: // 发布了文章或者视频
                postVideoOrArticleView.frame = CGRect(x: 15, y: 0, width: screenWidth - 30, height: middleView.height)
                if dongtai.group.group_id != 0 {
                    postVideoOrArticleView.group = dongtai.group
                } else if dongtai.origin_group.group_id != 0 {
    ...
    override func awakeFromNib() {
        ...
        moreButton.theme_setImage("images.morebutton_dynamic_press_14x8_", forState: .selected)
