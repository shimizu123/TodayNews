...
struct NewsTitleTable {
    ...
    // 插入一组数据
    func insert(titles: [HomeNewsTitle]) { _ = titles.map { insert(title: $0) } }
    
    // 插入一条数据
    func insert(title: HomeNewsTitle) {
        // 如果数据库中该条数据数据不存在，就插入
        if !exist(title: title) {
            let insert = news_title.insert(category <- title.category.rawValue, tip_new <- Int64(title.tip_new), default_add <- Int64(title.default_add), web_url <- title.web_url, concern_id <- title.concern_id, icon_url <- title.icon_url, flags <- Int64(title.flags), type <- Int64(title.type), name <- title.name, selected <- title.selected)
            // 插入数据
    ...
    // 判断数据库中某一条数据是否存在
    func exist(title: HomeNewsTitle) -> Bool {
        // 取出该新闻分类标题的数据
    ...
    // 更新数据
    func update(newsTitle: HomeNewsTitle) {
        // 取出数据库中数据
