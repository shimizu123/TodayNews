...
protocol NetworkToolProtocol {
    ...
    static func loadApiNewsFeeds(category: NewsTitleCategory, ttFrom: TTFrom, completionHandler:  @escaping (_ maxBeHotTime: TimeInterval, _ news: [NewsModel]) -> ())
    // MARK: 获取首页、视频、小视频的新闻列表数据,加载更多
    static func loadMoreApiNewsFeeds(category: NewsTitleCategory, ttFrom: TTFrom, maxBeHotTime: TimeInterval, listCount: Int, completionHandler: @escaping (_ news: [NewsModel]) -> ())
    // MARK: 获取一般新闻详情数据
    ...
    static func loadDongtaiDetailUserDiggList(id: Int, offset: Int, completionHandler: @escaping (_ diggs: [DongtaiUserDigg]) -> ())
    // MARK: 获取问答的列表数据（提出了问题）
    static func loadProposeQuestionBrow(qid: Int, enterFrom: WendaEnterFrom, completionHandler: @escaping (_ wenda: Wenda) -> ())
    // MARK: 获取问答的列表数据（提出了问题），加载更多
    static func loadMoreProposeQuestionBrow(qid: Int, offset: Int, enterFrom: WendaEnterFrom, completionHandler: @escaping (_ wenda: Wenda) -> ())
...
extension NetworkToolProtocol {
        ...
        Alamofire.request(url, parameters: params).responseJSON { (response) in
            ...
            if let value = response.result.value {
                ...
                if let data = dataDict!["data"]?.arrayObject {
                    completionHandler(data.flatMap({ HomeNewsTitle.deserialize(from: $0 as? NSDictionary)
                    }))
                }
            }
        }
        ...
        Alamofire.request(url, parameters: params).responseJSON { (response) in
            ...
            if let value = response.result.value {
                ...
                if let data = json["data"].dictionaryObject {
                    completionHandler(data["homepage_search_suggest"] as! String)
                }
            }
        }
    }
        ...
        Alamofire.request(articleURL).responseString { (response) in
            ...
            if let value = response.result.value {
                ...
                if value.contains("BASE_DATA.galleryInfo =") { // 则是图文详情
                    ...
                    let titleEndIndex = value.range(of: ",\"sub_titles\"")!.lowerBound
                    let sub_abstracts = value[Range(uncheckedBounds: (lower: titleStartIndex, upper: titleEndIndex) )]
                    ...
                    let content = value[Range(uncheckedBounds: (lower: startIndex, upper: endIndex) )]
                    ...
                    html += "<meta content='width=device-wdith,initial-scale=1.0,maximum-scale=3.0,user-scalable=0;' name='viewport' />"
        ...
        Alamofire.request(url).responseString { (response) in
            ...
            if let value = response.result.value {
                if value.contains("BASE_DATA.galleryInfo =") {
                    ...
                    let dicts = try! JSONSerialization.jsonObject(with: data, options: .mutableContainers) as! [Any]
                    let images = dicts.flatMap({ NewsDetailImage.deserialize(from: $0 as? NSDictionary)
                    })
                    // 获取 子标题
                    let titleStartIndex = value.range(of: "\\\"sub_abstracts\\\":")!.upperBound
                    let titleEndIndex = value.range(of: ",\\\"sub_titles\\\"")!.lowerBound
                    ...
                    let titleData = titleSubstring2.data(using: .utf8)! as Data
                    let abstracts = try! JSONSerialization.jsonObject(with: titleData, options: .mutableContainers) as! [String]
    ...
    static func loadMyCellData(completionHandler: @escaping (_ sections: [[MyCellModel]]) -> ()) {
        ...
        Alamofire.request(url, parameters: params).responseJSON { (response) in
            ...
            if let value = response.result.value {
                ...
                myConcerns.append([MyCellModel.deserialize(from: "{\"text\":\"我的关注\",\"grey_text\":\"\"}")!])
                
                if let data = json["data"].dictionary {
                    if let sections = data["sections"]?.arrayObject {
                        myConcerns += sections.flatMap({ (item)  in
                            (item as! [Any]).flatMap({
                                MyCellModel.deserialize(from: $0 as? NSDictionary)
                            })
                        })
                        completionHandler(myConcerns)
    ...
    static func loadRelationUserRecommend(userId: Int, completionHandler: @escaping (_ concerns: [UserCard]) -> ()) {
        ...
        let params = ["divice_id": device_id,
            ...
            if let value = response.result.value {
                ...
                if let data = json["data"].dictionary {
                    ...
                    if let datas = data["data"]?.arrayObject {
                        completionHandler(max_cursor!, datas.flatMap({ UserDetailDongtai.deserialize(from: $0 as? NSDictionary)
                        }))
                    }
                }
            }
    ...
    static func loadUserDetailNormalDongtaiComents(groupId: Int, offset: Int, count: Int, completionHandler: @escaping (_ comments: [DongtaiComment]) -> ()) {
        ...
        Alamofire.request(url, method: .post, parameters: params).responseJSON { (response) in
            ...
            if let value = response.result.value {
                ...
                if let datas = json["data"].arrayObject {
                    completionHandler(datas.flatMap({ DongtaiComment.deserialize(from: ($0 as! NSDictionary)["comment"] as? NSDictionary)
                    }))
                }
            }
        }
                ...
                if let data = json["data"].dictionary {
                    if let datas = data["data"]?.arrayObject {
                        completionHandler(datas.flatMap({
                            DongtaiComment.deserialize(from: $0 as? NSDictionary)
                        }))
                    }
                }
            }
        ...
        Alamofire.request(url, parameters: params).responseJSON { (response) in
            ...
            if let value = response.result.value {
                ...
                if let data = json["data"].dictionary {
                    if let datas = data["data"]?.arrayObject {
                        completionHandler(datas.flatMap({
                            DongtaiUserDigg.deserialize(from: $0 as? NSDictionary)
                        }))
                    }
                }
            }
    ...
    static func loadProposeQuestionBrow(qid: Int, enterFrom: WendaEnterFrom, completionHandler: @escaping (_ wenda: Wenda) -> ()) {
        ...
        let params = ["qid": qid,
                      "enter_form": enterFrom] as [String : Any]
        Alamofire.request(url, method: .post, parameters: params).responseJSON { (response) in
            // 网络错误的提示信息
            guard response.result.isSuccess else { return }
            if let value = response.result.value {
