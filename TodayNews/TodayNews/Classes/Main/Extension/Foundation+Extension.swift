...
extension TimeInterval {
    ...
    func convertString() -> String {
        // 把获取到的秒数转换成具体的时间
        let creatDate = Date(timeIntervalSince1970: self)
        // 获取当前日历
        let calendar = Calendar.current
        // 获取日期的年份
        let comps = calendar.dateComponents([.year, .month, .day, .hour, .minute, .second], from: creatDate, to: Date())
        // 日期格式
        let formatter = DateFormatter()
        // 判断当前日期是否为今年
        guard creatDate.isThisYear() else {
            formatter.dateFormat = "yyyy-MM-dd HH:mm:ss"
            return formatter.string(from: creatDate)
        }
        // 是否是前天
        if creatDate.isBeforeYesterday() {
            formatter.dateFormat = "前天 HH:mm"
            return formatter.string(from: creatDate)
        } else if creatDate.isYesterday() || creatDate.isToday() {
            // 判断是否是今天或者昨天
            if comps.hour! >= 1 {
